...
class Test_Time_Series_1:
        ...
        for ufunc in ufuncs.values():
            ...
            if ufunc.__name__ in ['bitwise_and', 'bitwise_or', 'bitwise_xor',
                ...
                if tsd.ndim > 1:
                    ...
                    assert np.allclose(out.index, tsd.index)
                    assert np.allclose(out.values, ufunc(tsd.values, x))
                ...
                assert isinstance(out, tsd.__class__)
                assert np.allclose(out.index, tsd.index)
                assert np.allclose(out.values, ufunc(tsd.values))
                a.append(ufunc.__name__)
                ...
                assert isinstance(out, tsd.__class__)
                assert np.allclose(out.index, tsd.index)
                assert np.allclose(out.values, ufunc(tsd.values, 1))
                ...
                assert isinstance(out, tsd.__class__)
                assert np.allclose(out.index, tsd.index)
                assert np.allclose(out.values, ufunc(tsd.values, 0.2, 0.6))

                a.append(ufunc.__name__)

    def test_funcs(self, tsd):
        # a = np.asarray(tsd)
        # assert np.allclose(a, tsd.values)
        ...
        if tsd.nap_class == "TsdTensor":
            ...
            assert np.allclose(a.values, np.reshape(tsd.values, (tsd.shape[0], np.prod(tsd.shape[1:]))))

        a = np.ravel(tsd.values)
        assert np.allclose(a, np.ravel(tsd))

        a = np.transpose(tsd.values)
        assert np.allclose(a, np.transpose(tsd))
        ...
        #     assert isinstance(a, nap.TsdFrame)
        #     assert np.allclose(a.values, tsd.values[:,0:2])

        a = np.isnan(tsd)
        assert isinstance(a, tsd.__class__)
        assert np.allclose(a.values, np.isnan(tsd.values))
    ...
    def test_split(self, tsd):
        ...
        for i in range(4):
            assert np.allclose(a[i].values, b[i])
            assert np.allclose(a[i].index, c[i])
        ...
        if tsd.ndim == 2:
            ...
            for i in range(1):
                assert np.allclose(a[i].values, b[i])
                assert np.allclose(a[i].index, tsd.index)
    ...
    def test_operators(self, tsd):
        ...
        a = tsd + 0.5
        assert isinstance(a, tsd.__class__)
        assert np.allclose(tsd.index, a.index)
        ...
        assert isinstance(a, tsd.__class__)
        assert np.allclose(tsd.index, a.index)
        assert np.allclose(a.values, v**0.5)
    ...
    def test_slice(self, tsd):
        a = tsd[0:10]
        assert isinstance(a, tsd.__class__)
        assert np.allclose(tsd.index[0:10], a.index)
        assert np.allclose(tsd.values[0:10], a.values)

        if tsd.nap_class == "TsdTensor":
            a = tsd[:,0:2,2:4]
            assert isinstance(a, nap.TsdTensor)
            assert np.allclose(tsd.index, a.index)
            assert np.allclose(tsd.values[:,0:2,2:4], a.values)

            a = tsd[:,0]
            assert isinstance(a, nap.TsdFrame)
            assert np.allclose(tsd.index, a.index)
            assert np.allclose(tsd.values[:,0], a.values)

            a = tsd[:,0,0]
            assert isinstance(a, nap.Tsd)
            assert np.allclose(tsd.index, a.index)
            assert np.allclose(tsd.values[:,0,0], a.values)
        ...
        if tsd.nap_class == "TsdFrame":
            ...
            assert isinstance(a, nap.TsdFrame)
            assert np.allclose(tsd.index, a.index)
            assert np.allclose(tsd.values[:,[0,2]], a.values)
    ...
    def test_searching(self, tsd):
        ...
        for func in [np.argwhere]:
            a = func(tsd)
            assert np.allclose(a, func(tsd.values))
    ...
    def test_concatenate(self, tsd):
        ...
        assert np.allclose(a.values, np.concatenate((tsd.values, tsd2.values)))
        time_support = nap.IntervalSet(start=[0, 150], end=[99, 249])
        assert np.allclose(time_support.values, a.time_support.values)

        b = np.concatenate((tsd, tsd2), axis=0)
        assert np.allclose(a.values, b.values)
        assert np.allclose(a.index.values, b.index.values)

        c = np.concatenate((tsd, tsd2), 0)
        assert np.allclose(a.values, c.values)
        assert np.allclose(a.index.values, c.index.values)

        d = np.concatenate((tsd, tsd2))
        assert np.allclose(a.values, d.values)
        assert np.allclose(a.index.values, d.index.values)
        ...
        if tsd.ndim >= 2:
            ...
            assert isinstance(out, tsd.__class__)
            assert np.allclose(out.values,np.concatenate((tsd.values, tsd.values), 1))
            assert np.allclose(tsd.index.values,out.index.values)
        ...
        if tsd.ndim == 3:
            ...
            assert isinstance(out, tsd.__class__)
            assert np.allclose(out.values,np.concatenate((tsd.values, tsd.values), 2))
            assert np.allclose(tsd.index.values,out.index.values)
