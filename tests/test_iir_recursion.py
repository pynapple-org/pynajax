...
from pynajax.iir_filtering import sosfiltfilt
...
def test_filter_recursion_single_band(wn, order, btype, n_sig):
    ...
    out_jax = sosfiltfilt(sos, np.arange(sig.shape[0]), sig, [0], [1000])
...
def test_filter_recursion_single_band_multiepoch_tsdframe(wn, order, btype, n_sig, ep):
    ...
    out_jax = sosfiltfilt(sos, sig.t, sig.d, sig.time_support.start, sig.time_support.end)
    assert np.allclose(out_jax, out_sci)
